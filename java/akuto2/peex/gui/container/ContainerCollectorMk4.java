...
public class ContainerCollectorMk4 extends Container{
	...
	public ContainerCollectorMk4(InventoryPlayer invPlayer, TileEntityCollectorMk4 collector) {
		tile = collector;
		initSlots(invPlayer);
	}
	...
	protected void initSlots(InventoryPlayer invPlayer) {
		...
		addSlotToContainer(new SlotGhost(aux, TileEntityCollectorMk4.LOCK_SLOT, 187, 36, SlotPredicates.COLLECTOR_LOCK));

		for(int i = 0; i < 3; i++) {
			for(int j = 0; j < 9; j++) {
				addSlotToContainer(new Slot(invPlayer, j + i * 9 + 9, 30 + j * 18, 84 + i * 18));
			}
		}

		for(int i = 0; i < 9; i++) {
			addSlotToContainer(new Slot(invPlayer, i, 30 + i * 18, 142));
		}
	}
