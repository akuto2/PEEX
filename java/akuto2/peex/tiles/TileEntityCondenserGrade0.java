...
import moze_intel.projecte.gameObjs.tiles.WrappedItemHandler.WriteMode;
...
public class TileEntityCondenserGrade0 extends TileEntity implements ITickable{
	...
	protected IItemHandler createAutomationInventory() {
		return new WrappedItemHandler(inputInventory, WriteMode.IN_OUT) {
			@Override
	...
	private void updateChest() {
		if(++ticksSinceSync % 20 * 4 == 0) {
			world.addBlockEvent(pos, ObjHandlerPEEX.condenserGrade0, 1, numPlayersUsing);
		}
	...
	private void pushToInventories() {
		for(EnumFacing face : EnumFacing.VALUES) {
			...
			for(int i = 0; i < getOutput().getSlots(); i++) {
				...
				int successfullyTransferred = extractTest.getCount() - (remainderTest.isEmpty() ? 0 : remainderTest.getCount());
